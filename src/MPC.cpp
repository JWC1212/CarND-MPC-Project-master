  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // the Solver function below.
      ...
      for (int t = 0; t < N; t++) {
          ...
          fg[0] += CppAD::pow(vars[v_start + t] - 17.88, 2);
      }
      for (int t = 0; t < N - 1; t++) {
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  double x = state[0] + state[3] * cos(state[2]) * 0.1; //latency is taken account into.
  double y = state[1] + state[3] * sin(state[2]) * 0.1;
  double psi = state[2] + state[3] / Lf * state[6] * 0.1;
  double v = state[3] + state[7] * 0.1;
  double cte = state[4] + state[3] * sin(state[5]) * 0.1;
  double epsi = state[5] + state[3] / Lf * state[6] * 0.1;
